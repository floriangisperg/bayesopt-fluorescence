...
class BaseTransformation:
    def __init__(self, lower_bound, upper_bound): # TODO: ev check if lower<upper
...
class ReciprocalScaler(BaseTransformation):
    ...
    def physical_to_unit(self, x: ArrayLike) -> ArrayLike:
        return (self.upper_bound - x) / (self.upper_bound - self.lower_bound)
...
class ParameterTransformer:
    def __init__(
        ...
        for i, name in enumerate(self.parameter_names):
            ...
            if (model_transform_cls or user_transform_cls) is None:
    ...
    def _as_2d(x: ArrayLike) -> tuple[ArrayLike, bool]:
        if torch.is_tensor(x):
            ...
            if x.ndim == 2:
                return x, False
        raise ValueError("Expected shape (d,) or (n, d).")

    @staticmethod
    def _restore_shape(x: ArrayLike, squeeze: bool) -> ArrayLike:
        return x[0] if squeeze else x

    def physical_to_unit_model(self, x: ArrayLike) -> ArrayLike:
        x2d, squeeze = self._as_2d(x)
        z = x2d.clone() if torch.is_tensor(x2d) else x2d.copy()
        for i, spec in enumerate(self.specs):
            z[:, i] = spec.model_transform.physical_to_unit(x2d[:, i])
        return self._restore_shape(z, squeeze)

    def unit_to_physical_model(self, z: ArrayLike) -> ArrayLike:
        z2d, squeeze = self._as_2d(z)
        x = z2d.clone() if torch.is_tensor(z2d) else z2d.copy()
        for i, spec in enumerate(self.specs):
            x[:, i] = spec.model_transform.unit_to_physical(z2d[:, i])
        return self._restore_shape(x, squeeze)

    def physical_to_unit_user(self, x: ArrayLike) -> ArrayLike:
        x2d, squeeze = self._as_2d(x)
        z = x2d.clone() if torch.is_tensor(x2d) else x2d.copy()
        for i, spec in enumerate(self.specs):
            z[:, i] = spec.user_transform.physical_to_unit(x2d[:, i])
        return self._restore_shape(z, squeeze)

    def unit_to_physical_user(self, z: ArrayLike) -> ArrayLike:
        z2d, squeeze = self._as_2d(z)
        x = z2d.clone() if torch.is_tensor(z2d) else z2d.copy()
        for i, spec in enumerate(self.specs):
            x[:, i] = spec.user_transform.unit_to_physical(z2d[:, i])
        return self._restore_shape(x, squeeze)
